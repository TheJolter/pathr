...
export default function getUsdcBalance({
  ...
  return new Promise((resolve, reject) => {
    ...
    if (chian.chainType === 'evm') {
      const provider = new ethers.JsonRpcProvider(chian.rpc)
      ...
      contract.balanceOf(address).then(balance=>{
        resolve(ethers.formatUnits(balance, 6))
      }).catch(error=>{
        reject(error)
      })
      return
    ...
    if (chian.chainType === 'cosmos') {
      fetch(`${chian.lcd}/cosmos/bank/v1beta1/balances/${cosmosAddrConvertor(address, chian.prefix)}`).then(res=>res.json()).then(({balances})=>{ // get [] for address with no balance
        resolve(ethers.formatUnits(balances.find((item:any)=>item.denom===chian.usdcAddress)?.amount ?? '0', 6))
      }).catch(error=>{
        reject(error)
      })
      return
